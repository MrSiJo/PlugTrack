...
async def test_answer_returns_model_text():
    ...
    async with httpx.AsyncClient(
        transport=httpx.MockTransport(_resp("You spent £42.18 this month."))
    ) as c:
        text, usage = await answer_usage_question(
            "spend this month?", snap, api_key="sk", model="gpt-5-mini", client=c
        )
