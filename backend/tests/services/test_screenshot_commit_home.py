...
@pytest.mark.asyncio
async def test_home_mycupra_only_derives_kwh_and_costs_home_rate(
...
async def test_commit_sets_actual_charge_seconds(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        cs = await commit_merged_session(
            s, user_id=user_id, car_id=car_id, merged=_merged(actual_charge_seconds=13783)
        )
        await s.commit()
        await s.refresh(cs)
...
@pytest.mark.asyncio
async def test_home_with_granny_uses_delivered_and_matches_location(
    test_sessionmaker, seeded_user_car
):
    ...
    async with test_sessionmaker() as s:
        cs = await commit_merged_session(
            ...
            car_id=car_id,
            merged=_merged(
                energy_kwh=9.3, location_name="home", source_kinds=["mycupra", "granny"]
            ),
        )
        await s.commit()
        await s.refresh(cs)
...
async def test_commit_sets_odometer_miles_to_km(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        cs = await commit_merged_session(
            ...
            car_id=car_id,
            merged=_merged(
                energy_kwh=9.3, location_name="home", odometer=12345, odometer_unit="mi"
            ),
        )
        await s.commit()
        await s.refresh(cs)
...
async def test_commit_respects_explicit_km(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        cs = await commit_merged_session(
            ...
            car_id=car_id,
            merged=_merged(
                energy_kwh=9.3, location_name="home", odometer=20000, odometer_unit="km"
            ),
        )
        await s.commit()
        await s.refresh(cs)
...
async def test_commit_defaults_unit_from_setting(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        cs = await commit_merged_session(
            ...
            car_id=car_id,
            merged=_merged(
                energy_kwh=9.3, location_name="home", odometer=12345, odometer_unit=None
            ),
        )
        await s.commit()
        await s.refresh(cs)
...
async def test_commit_normalizes_unit_aliases(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        cs = await commit_merged_session(
            ...
            car_id=car_id,
            merged=_merged(
                energy_kwh=9.3, location_name="home", odometer=12345, odometer_unit="miles"
            ),
        )
        await s.commit()
        await s.refresh(cs)
    ...
    async with test_sessionmaker() as s:
        cs2 = await commit_merged_session(
            ...
            car_id=car_id,
            merged=_merged(
                start_at=__import__("datetime").datetime(
                    2026, 6, 14, 9, 0, tzinfo=__import__("datetime").timezone.utc
                ),
                energy_kwh=5.0,
                location_name="home",
                odometer=20000,
                odometer_unit="kilometres",
            ),
        )
        await s.commit()
        await s.refresh(cs2)
...
async def test_commit_creates_geocoded_location(test_sessionmaker, seeded_user_car, monkeypatch):
    ...
    merged = _merged(
        ...
        location_address="1 Fore Street, Lifton, PL16 0AA",
        location_short_name="Tesla Lifton",
        source_kinds=["tesla"],
    )
    async with test_sessionmaker() as s:
...
async def test_preview_does_not_geocode_or_create(test_sessionmaker, seeded_user_car, monkeypatch):
    ...
    merged = _merged(
        ...
        soc_end=None,
        location_name="Lifton",
        location_address="1 Fore Street, Lifton",
        location_short_name="Tesla Lifton",
        source_kinds=["tesla"],
    )
    async with test_sessionmaker() as s:
...
async def test_committed_odometer_drives_current_mileage(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        await commit_merged_session(
            ...
            car_id=car_id,
            merged=_merged(
                start_at=dt.datetime(2026, 6, 16, 9, 0, tzinfo=dt.UTC),
                energy_kwh=5.0,
                location_name="home",
                odometer=11000,
                odometer_unit="mi",
