...
_MYCUPRA = dict(source="mycupra", soc_start=67, soc_end=80,
...
async def _stage(ctx, ex, sha, user_id):
    await _stage_and_card(ctx, user_id=user_id, chat_id=9, extraction=ex,
                          usage=Usage(1, 1, 0), telegram_file_id=sha, message_id=1, sha=sha)
...
async def test_save_clears_card_so_next_charge_is_fresh(test_sessionmaker, seeded_user_car):
    ...
    await _stage(ctx, _ex(**_MYCUPRA, ), "m2", user_id)
