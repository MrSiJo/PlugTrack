...
def _merged(**over):
    base = dict(
        ...
        soc_start=56,
        soc_end=70,
        location_name="Land's End Car Park",
...
@pytest.mark.asyncio
async def test_commit_snapshots_location_network_when_session_blank(
...
@pytest.mark.asyncio
async def test_commit_leaves_network_blank_for_placeholder_location(
    test_sessionmaker, seeded_user_car
):
    ...
    async with test_sessionmaker() as s:
        s.add(
            Location(
                ...
                centroid_lng=-5.6,
                radius_m=100,
                default_charge_network="Unknown Network",
            )
        )
        await s.commit()
    merged = _merged(
