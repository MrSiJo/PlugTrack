...
def _csv_dict(
    *,
    sid="s1",
    started="2026-06-15T18:27:30Z",
    ended="2026-06-16T05:59:21Z",
    charging_s=41511,
    actual_s=13783,
    energy=6.0,
    isoc=67,
    ...
    return {
        ...
        "Session ended": ended,
        "Charging time (s)": str(charging_s),
        "Actual charging time (s)": str(actual_s),
        "Total energy (kWh)": f"{energy:.2f}",
        "Battery energy (kWh)": "",
        "Comfort energy (kWh)": "",
        "Initial state of charge (%)": f"{isoc:.2f}",
        "Final state of charge (%)": f"{fsoc:.2f}",
        "Total price": "",
        "Energy price": "",
        "Blocking fees": "",
        "Voucher amount used": "",
...
async def _add_session(
    sm, *, user_id, car_id, start_local, isoc, fsoc, kwh, charging_type="ac", **kw
):
    async with sm() as s:
        cs = ChargingSession(
            user_id=user_id,
...
async def test_update_existing_match_sets_actual_and_id_only(test_sessionmaker, seeded_user_car):
    ...
    sid = await _add_session(
        ...
        user_id=user_id,
        car_id=car_id,
        start_local=dt.datetime(2026, 6, 15, 19, 27),
        isoc=67,
        fsoc=80,
        kwh=10.74,
    )
...
async def test_insert_missing_creates_import_source(test_sessionmaker, seeded_user_car):
    ...
    rows = parse_csv_rows(
        [
            _csv_dict(
                sid="cupra-x",
                started="2026-06-14T15:30:05Z",
                ended="2026-06-15T05:39:07Z",
                actual_s=17536,
                energy=11.0,
                isoc=47,
                fsoc=63,
    ...
    async with test_sessionmaker() as s:
        cs = (
            await s.execute(
                select(ChargingSession).where(ChargingSession.telematics_session_id == "cupra-x")
            )
        ).scalar_one()
    ...
    async with test_sessionmaker() as s:
        n = (
            await s.execute(
                ...
                .where(ChargingSession.telematics_session_id == "cupra-x")
            )
        ).scalar_one()
        assert n == 1
...
async def test_same_day_multiples_match_distinctly(test_sessionmaker, seeded_user_car):
    ...
    a = await _add_session(
        ...
        user_id=user_id,
        car_id=car_id,
        start_local=dt.datetime(2026, 5, 27, 18, 56),
        isoc=60,
        fsoc=67,
        kwh=4.0,
    )
    b = await _add_session(
        ...
        user_id=user_id,
        car_id=car_id,
        start_local=dt.datetime(2026, 5, 27, 19, 24),
        isoc=67,
        fsoc=90,
        kwh=15.39,
    )
    rows = parse_csv_rows(
        [
            _csv_dict(
                sid="m1", started="2026-05-27T17:56:25Z", actual_s=653, energy=4.0, isoc=60, fsoc=67
            ),
            _csv_dict(
                sid="m2",
                started="2026-05-27T18:24:45Z",
                actual_s=863,
                energy=13.0,
                isoc=67,
                fsoc=90,
...
async def test_energy_endsoc_fallback_matches_divergent_start(test_sessionmaker, seeded_user_car):
    ...
    sid = await _add_session(
        ...
        user_id=user_id,
        car_id=car_id,
        start_local=dt.datetime(2026, 6, 8, 13, 35),
        isoc=78,
        fsoc=90,
        kwh=15.9,
        charging_type="dc",
    ...
    rows = parse_csv_rows(
        [
            _csv_dict(
                sid="d1",
                started="2026-06-08T12:49:00Z",
                ended="2026-06-08T13:08:00Z",
                actual_s=1059,
                energy=16.0,
                isoc=64,
                fsoc=90,
...
async def test_fallback_respects_distinct_end_soc_same_day(test_sessionmaker, seeded_user_car):
    ...
    await _add_session(
        ...
        user_id=user_id,
        car_id=car_id,
        start_local=dt.datetime(2026, 6, 12, 15, 25),
        isoc=56,
        fsoc=70,
        kwh=10.0,
    )
...
async def test_insert_assigns_free_location_by_date(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        cs = (
            await s.execute(
                select(ChargingSession).where(ChargingSession.telematics_session_id == "c1")
            )
        ).scalar_one()
