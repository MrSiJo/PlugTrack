...
async def test_reconcile_starts_and_stops(monkeypatch, test_sessionmaker):
    ...
    cfg = BotConfig(token="t", openai_key="k", model="gpt-5-mini", allowed={111}, user_id=1)
    state = {"cfg": cfg}
    ...
    monkeypatch.setattr(
        "plugtrack.services.telegram_manager.build_ingest_context",
...
async def test_reconcile_idempotent_no_restart(monkeypatch, test_sessionmaker):
    ...
    monkeypatch.setattr(
        "plugtrack.services.telegram_manager.load_bot_config", lambda sm: _coro(cfg)
    )
    monkeypatch.setattr(
        "plugtrack.services.telegram_manager.build_ingest_context",
        lambda c, *, sessionmaker, health_check, ai_enabled: object(),
...
def test_fingerprint_differs_on_ai_enabled():
    ...
    base = BotConfig(token="t", openai_key="k", model="m", allowed={1}, user_id=1, ai_enabled=False)
    enabled = BotConfig(
        token="t", openai_key="k", model="m", allowed={1}, user_id=1, ai_enabled=True
    )
