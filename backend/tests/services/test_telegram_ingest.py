...
async def test_photo_caption_fills_home_and_mileage(test_sessionmaker, seeded_user_car):
    ...
    await handle_photo(
        ctx, from_id=111, chat_id=9, message_id=1, file_id="x", caption="Home 11001mi"
    )
...
@pytest.mark.asyncio
async def test_photo_caption_mileage_does_not_clobber_found_location(
    ...
    ext_in = _ex_photo(
        source="tesla",
        has_cost=True,
        cost_total_pence=972,
        location_name="Dart Farm Village, Exeter",
        soc_start=None,
        soc_end=None,
        network="Tesla",
    )
...
def _merged():
    return MergedSession(
        start_at=dt.datetime(2026, 6, 12, 14, 25, tzinfo=dt.UTC),
        end_at=dt.datetime(2026, 6, 12, 14, 40, tzinfo=dt.UTC),
        energy_kwh=9.78,
...
def test_summarise_shows_actual_charge_time():
    ...
    m = MergedSession(
        start_at=dt.datetime(2026, 6, 15, 18, 27, tzinfo=dt.UTC),
        end_at=dt.datetime(2026, 6, 16, 5, 59, tzinfo=dt.UTC),
        energy_kwh=None,
        cost_total_pence=None,
        cost_per_kwh_pence=None,
        soc_start=67,
        ...
        peak_kw=2.0,
        confidence=0.95,
        actual_charge_seconds=3 * 3600 + 49 * 60,
        source_kinds=["mycupra"],
    )
    text = _summarise([m])
...
def test_summarise_shows_efficiency_and_location():
    ...
    m = MergedSession(
        start_at=dt.datetime(2026, 6, 15, 19, 27, tzinfo=dt.UTC),
        end_at=dt.datetime(2026, 6, 16, 6, 59, tzinfo=dt.UTC),
        energy_kwh=9.3,
        cost_total_pence=None,
        cost_per_kwh_pence=None,
        soc_start=67,
...
def test_summarise_itemised_home_card():
    ...
    m = MergedSession(
        start_at=dt.datetime(2026, 6, 18, 13, 17, tzinfo=dt.UTC),
        end_at=dt.datetime(2026, 6, 18, 17, 15, tzinfo=dt.UTC),
        energy_kwh=None,
        cost_total_pence=None,
        cost_per_kwh_pence=None,
        soc_start=67,
...
def test_summarise_dc_uses_plug_emoji_and_basis_label():
    ...
    m = MergedSession(
        start_at=dt.datetime(2026, 6, 13, 8, 43, tzinfo=dt.UTC),
        end_at=dt.datetime(2026, 6, 13, 9, 1, tzinfo=dt.UTC),
        energy_kwh=37.9,
        cost_total_pence=1706,
        cost_per_kwh_pence=None,
        soc_start=None,
        soc_end=None,
        location_name="Lifton",
        location_address=None,
        network="Tesla",
        peak_kw=62.0,
        confidence=0.95,
    ...
    text = _summarise(
        [m],
        projected=[{"kwh_added": 37.9, "cost_pence": 1706, "cost_basis": "override_total"}],
        unit="mi",
    )
    assert "🔌" in text
...
def test_summarise_low_confidence_warning():
    ...
    text = _summarise(
        [m],
        projected=[{"kwh_added": 9.3, "cost_pence": 178, "cost_basis": "location_rate"}],
        unit="mi",
    )
...
async def test_handle_text_runs_health_for_command():
    ...
    async def health(from_id):
        ...
        return HealthReport(
            checks=[Check("Telegram", True, "ok")], all_ok=True, usage_this_month=None
        )
...
async def test_handle_text_non_charge_falls_back_to_help(test_sessionmaker, seeded_user_car):
    ...
    async def extractor_text(text):
        return ExtractionResult(
            extraction=_ex_text(energy_kwh=None, location_name=None, start_at=None, confidence=0.0),
            usage=Usage(5, 5, 0),
        )
...
async def test_non_charge_text_routes_to_usage_answerer():
    ...
    ctx = ti.IngestContext(
        ...
        resolve_target=lambda: (1, 1),
        allowed_user_ids={42},
        extractor_text=fake_text_extractor,
        usage_answerer=fake_answerer,
    )
        ...
        projected=[
            {
                ...
                "cost_pence": 179,
                "cost_basis": "home_rate",
                "odometer_km": 10000 * 1.609344,
                "odometer_regressed": True,
                "existing_max_km": 12000 * 1.609344,
            }
        ],
        unit="mi",
    )
...
async def _seed_two_cars(test_sessionmaker):
    ...
    async with test_sessionmaker() as s:
        ...
        car2 = Car(
            ...
            model="Model 3",
            battery_kwh=75.0,
            nominal_efficiency_mi_per_kwh=4.5,
            provider="manual",
            active=True,
            name="Model3",
        )
...
async def test_carpick_callback_then_save_commits_with_chosen_car(test_sessionmaker):
    ...
    await handle_callback(
        ctx, from_id=111, callback_id="cb_pick", data=f"carpick:{car_id_2}", chat_id=9
    )
    ...
    all_cb = [
        btn["callback_data"] for row in save_card["reply_markup"]["inline_keyboard"] for btn in row
    ]
...
async def test_second_photo_reuses_pending_car_choice(test_sessionmaker):
    ...
    ctx2 = _stub_ctx(
        tg, test_sessionmaker, car_id_1, user_id, _ex_photo(start_at="2026-06-17T18:00:00")
    )
...
async def test_zero_active_cars_sends_friendly_message(test_sessionmaker):
    ...
    from plugtrack.models import Car, ScreenshotImport, User
    from sqlalchemy import select
...
@pytest.mark.asyncio
async def test_save_after_bot_restart_resolves_car_from_single_active(
    test_sessionmaker, seeded_user_car
):
