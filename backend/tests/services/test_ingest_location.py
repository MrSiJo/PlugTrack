...
@pytest.mark.asyncio
async def test_resolve_falls_back_to_postcode_when_address_misses(
    ...
    prov = MapProvider(
        {"PL16 0AA": GeocodeResult(address="Lifton", provider="fake", lat=50.6434, lng=-4.2841)}
    )
    async with test_sessionmaker() as s:
        loc_id = await resolve_ingested_location(
            s,
            user_id=user_id,
            place_name="Tesla Lifton",
            raw_label="Lifton",
            address="1 Fore Street, Lifton, United Kingdom, PL16 0AA",
            network="Tesla",
            provider=prov,
        )
        await s.commit()
...
async def test_resolve_creates_named_location(test_sessionmaker, seeded_user_car):
    ...
    async with test_sessionmaker() as s:
        loc_id = await resolve_ingested_location(
            ...
            place_name="Tesla Lifton",
            raw_label="Lifton",
            address="1 Fore Street, Lifton, PL16 0AA",
            network="Tesla",
            provider=prov,
        )
        await s.commit()
...
@pytest.mark.asyncio
async def test_resolve_matches_existing_within_radius_and_fills_only_empty(
    test_sessionmaker, seeded_user_car
):
    user_id, _ = seeded_user_car
    async with test_sessionmaker() as s:
        s.add(
            Location(
                user_id=user_id,
                name="Tesla Lifton",
                centroid_lat=50.6437,
                centroid_lng=-4.2846,
                radius_m=100,
                default_charge_network=None,
            )
        )
        await s.commit()
    ...
    async with test_sessionmaker() as s:
        loc_id = await resolve_ingested_location(
            s,
            user_id=user_id,
            place_name="SHOULD NOT OVERWRITE",
            raw_label="Lifton",
            address="Lifton",
            network="Tesla",
            provider=prov,
    ...
    async with test_sessionmaker() as s:
        s.add(
            ChargingSession(
                ...
                user_label="Lifton",
                notes="1 Fore Street, Lifton, PL16 0AA",
                source="import",
            )
        )
        await s.commit()
