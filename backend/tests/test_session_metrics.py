...
@pytest.mark.asyncio
async def test_unconfirmed_sessions_excluded_from_observed_efficiency(
    test_sessionmaker,
):
    """Unconfirmed rows are excluded from _observed_mi_per_kwh so they cannot
    corrupt the efficiency calibration used for savings (spec §4).

    Setup: two confirmed odometer legs implying an observed efficiency of
    3.0 mi/kWh (nominal is 5.0, far away so the two are clearly distinct).
    An interleaved unconfirmed row with a contradictory odometer / SoC must
    not distort the result.
    """
    async with test_sessionmaker() as s:
        s.add(User(id=1, username="alice", password_hash="x"))
        # Nominal 5.0 deliberately far from the expected observed 3.0.
        _seed_car(s, battery_kwh=50.0, mi_per_kwh=5.0)
        _add_petrol_settings(s, p_per_litre=150.0, mpg=50.0)

        # Two confirmed legs each 120.7008 km = 75 mi, consuming 25 kWh →
        # observed = (75+75) mi / (25+25) kWh = 3.0 mi/kWh.
        s.add(ChargingSession(
            id=1, user_id=1, car_id=1, date=date(2026, 5, 1),
            start_soc=30, end_soc=80, kwh_added=25.0,
            odometer_at_session_km=1000.0,
        ...
        # Unconfirmed row: bizarre odometer (implausibly large jump) and SoC
        # values that would wreck the efficiency calculation if included.
        # kwh_added=0.0 (column is NOT NULL) — unconfirmed rows have no metered
        # energy; the odometer value is what matters for the corruption test.
        s.add(ChargingSession(
            id=4, user_id=1, car_id=1, date=date(2026, 5, 7),
            start_soc=10, end_soc=95, kwh_added=0.0,
            odometer_at_session_km=50000.0,  # would imply absurd leg if included
            cost_basis="unknown", source="unconfirmed",
        ))
        # Estimated session — no odometer; savings should use observed 3.0.
        s.add(ChargingSession(
            id=5, user_id=1, car_id=1, date=date(2026, 5, 12),
            start_soc=60, end_soc=90, kwh_added=18.0, kwh_calculated=15.0,
            cost_pence=300, cost_basis="home_rate", source="manual",
        ))
        await s.commit()
        ...
        # Must still be 3.0 — the unconfirmed row did not corrupt the result.
        assert observed == pytest.approx(3.0, abs=1e-4)

        cs = await s.get(ChargingSession, 5)
        m = await compute_session_metrics(s, cs)
        # Savings must use the uncontaminated observed efficiency (3.0), not
        # the nominal (5.0) which would give very different miles.
        assert m.comparison_basis == "estimated"
        assert m.miles_since_previous == float(round(15.0 * 3.0))
        assert m.miles_since_previous != float(round(15.0 * 5.0))
