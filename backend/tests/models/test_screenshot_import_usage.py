...
async def test_usage_columns_roundtrip(test_sessionmaker):
    ...
    async with test_sessionmaker() as s:
        row = ScreenshotImport(
            user_id=1, image_sha256="abc", source="osprey", extracted={},
            status="staged", input_tokens=2400, output_tokens=410, reasoning_tokens=0,
        )
        s.add(row)
        await s.commit()
...
async def test_additive_migration_idempotent(test_engine):
    ...
    async with test_engine.begin() as conn:
        cols = {r[1] for r in (await conn.execute(text("PRAGMA table_info(screenshot_import)"))).all()}
