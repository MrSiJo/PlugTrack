...
async def test_get_settings_redacts_secrets(authed_client, test_sessionmaker):
    ...
    async with test_sessionmaker() as session:
        result = await session.execute(select(Setting).where(Setting.key == "openai_api_key"))
...
async def test_put_setting_requires_auth(seeded_client):
    r = await seeded_client.put("/api/settings", json={"key": "currency", "value": "EUR"})
    assert r.status_code == 401


@pytest.mark.asyncio
async def test_put_setting_requires_csrf(authed_client):
    r = await authed_client.put("/api/settings", json={"key": "currency", "value": "EUR"})
...
async def test_put_setting_updates_value(authed_client, test_sessionmaker):
    ...
    assert r.status_code == 200, r.text
    async with test_sessionmaker() as session:
        result = await session.execute(select(Setting).where(Setting.key == "currency"))
        row = result.scalar_one()
...
async def test_put_setting_encrypts_secret(authed_client, test_sessionmaker):
    ...
    async with test_sessionmaker() as session:
        ...
        decrypted = decrypt_secret(row.value, "test-secret-key-for-tests-only-padding-padding")
