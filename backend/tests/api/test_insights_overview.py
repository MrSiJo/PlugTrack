...
async def _seed_car_and_session(
    sm, *, when, kwh, cost_pence, ctype="ac", network=None, odometer_km=None
):
    async with sm() as s:
...
async def test_overview_shape(authed_client, test_sessionmaker):
    ...
    assert data["over_time"] == [
        {"period": "2026-06-02", "spend_pence": 1500, "kwh": 30.0, "sessions": 1}
    ]
...
async def test_overview_granularity_resolves_from_window(authed_client, test_sessionmaker):
    await _seed_car_and_session(
        test_sessionmaker, when=dt.date(2026, 3, 1), kwh=10.0, cost_pence=200
    )
