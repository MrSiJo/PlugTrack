...
describe('AdminPage — master-detail layout', () => {
  ...
  it('renders all 8 left-rail nav items', () => {
    renderAdminPage()
    const expectedKeys = ['telegram', 'ai', 'geocoding', 'preferences', 'maintenance', 'locations', 'cars', 'mcp']
    for (const key of expectedKeys) {
      expect(screen.getByTestId(`admin-nav-${key}`)).toBeInTheDocument()
    }
  })

  it('shows the telegram integration card by default (no hash)', () => {
  ...
  it('switches to ai integration when nav item is clicked', async () => {
  ...
  it('shows only one integration card at a time', async () => {
    ...
    // Default: telegram visible, others not
    expect(screen.getByTestId('integration-telegram')).toBeInTheDocument()
  ...
  it('falls back to the default integration for an unknown hash', () => {
